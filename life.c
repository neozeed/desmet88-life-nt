...
/*Modified 6/10/2012 to support NT console IO directly*/
	...
	char *argv[];
{
	...
	SetConsoleTitle("DeSmet C: LIFE");
	scr_setup();
	scr_clr();
	instruct();
	ndel = 100; /*getspeed();              /*kj1*/
	setup();
...
create(char suspend) {           /*      see if need to create or kill cells   */
	char ch,wait;

	while (1==2){ /*  ((ch=scr_csts()) || suspend) {**/
		ch=scr_csts();
		switch (ch) {
		case up_char:   if(extended==1){crow=crow ? crow-1: ROWS-1;}
							break;
		case down_char: if(extended==1){crow=crow == ROWS-1 ? 0: crow+1;}
							break;
		case left_char: if(extended==1){ccol=ccol ? ccol-1: COLS-1;}
							break;
		case right_char: if(extended==1){ccol=ccol == COLS-1 ? 0: ccol+1;}
							break;
